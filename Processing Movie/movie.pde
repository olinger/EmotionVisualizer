...
void setup()
{
  size(300,300);
  createUI();
  lines=loadStrings("saharaPCA.csv");
  selectSubject=5;
  selectX=1;
  selectY=1;
}
...
public class PlayApp extends PApplet
{
  ...
  {  
    ...
    Map<Integer,String> emotion_map = new HashMap<Integer, String>();
  ...
  */
  ...
  public void drawKey()
  {
    Boundary keyBound = new Boundary(innerBox.xl,500,innerBox.yh+15,40);
    ...
    for (Integer key : color_map.keySet()) 
    {
      ...
      i+=100;
      textSize(15);
     // text(color_map,35,37);
      text("y component",165,37);
      text("mode",35,258);
      
    }
    //color map<int, int> is emotion number to color num
    //get emtion label from emotion number from emotion map
    //draw small rectangles for each emotion in dataset
    //needs a loop
